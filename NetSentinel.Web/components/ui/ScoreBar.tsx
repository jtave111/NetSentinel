...
export function ScoreBar({score}:{score:number}){
  ...
  const c = cvssColor(safeScore);
  return(
    <div className="flex items-center gap-2">
      <div className="flex-1 h-[2px] min-w-[40px]" style={{background:"var(--b0)"}}>
        <div className={cn("h-full",F[c])} style={{width:`${safeScore*10}%`}}/>
      </div>
      <span className={cn("font-mono text-[11px] font-medium min-w-[22px]",N[c])}>{safeScore.toFixed(1)}</span>
    </div>
  );
}